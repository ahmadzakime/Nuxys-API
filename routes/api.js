...
let { igstalk } = require(__path + '/lib/apidl.js');
...
// Api Features :
        ...
        var randomTextNumber = random+randomlagi+'------Zaki-Api'+'Storage';
       ...
       fetch(encodeURI(`https://registry.npmjs.org/${query}`))
        ...
        var result = data;
             res.json({
                 ...
                 creator : `${creator}`,
                 result,
                 message : `Jangan Lupa Follow Ig @ahmadzaki_yo ${creator}`
             })
         })
         .catch(e => {
         	res.sendFile(error)
...
router.get('/stalk/ig', async (req, res, next) => {
    ...
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "Masukan parameter username"})
       ...
       zaki.igstalk(username).then(async (data) => {
	res.json({
	status: true,
	creator: `${creator}`,
	result:	data
